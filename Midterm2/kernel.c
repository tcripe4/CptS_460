...
int body()
{
	...
	while (1) {
		...
		kprintf("proc %d running, command = [switch|kfork|exit|sleep|wakeup|wait|ps|producer|consumer] : ", running->pid);
		kgets(cmd);
		printf("\n");
        ...
        {
            kprintf("Now producing \n");
            Producer();
        }
        if (strcmp(cmd, "consume") == 0)
        {
            kprintf("Now consuming \n");
            Consumer();
            kprintf("\n");
        }
	}
